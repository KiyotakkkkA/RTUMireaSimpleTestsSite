import {
    buildFullAnswerSystemPrompt,
    fillTestSystemPrompt,
} from "../data/prompts";
import { AnswerCheckingTool, TestFillingTool } from "../data/tools";

import type {
    FullAnswerModelEvaluation,
    OllamaChatResponse,
} from "../types/AI";
...
export const OllamaService = {
    fillTestFromText: async (input: { text: string }) => {
        ...
        if (!model) {
            throw new Error(
                "Ollama model is not configured (REACT_APP_OLLAMA_MODEL)",
            );
        }
        ...
        if (!res.ok) {
            ...
            throw new Error(
                `Ollama /api/chat failed: ${res.status} ${res.statusText}${text ? ` - ${text}` : ""}`,
            );
        }
        ...
        try {
            ...
            throw new Error(
                "Ollama did not return tool_calls(fill_test) or valid JSON fallback",
            );
        }
    },
    gradeFullAnswer: async (input: {
        ...
        const toolCall = data.message.tool_calls?.find(
            (c) => c.function?.name === "check_answer",
        );
        if (toolCall) {
            const scorePercent = clampPercent(
                Number(toolCall.function.arguments?.scorePercent),
            );
            const comment = String(
                toolCall.function.arguments?.comment ?? "",
            ).trim();
        ...
        try {
            ...
            throw new Error(
                "Ollama did not return tool_calls(check_answer) or valid JSON fallback",
            );
        }
    },
};
