...
export const QuestionFilesService = {
  upload: async (questionId: number, files: File[]): Promise<QuestionFile[]> => {
    const form = new FormData();
    files.forEach((file) => form.append('files[]', file));

    const response = await api.post<{ files: QuestionFile[] }>(`/workbench/questions/${questionId}/files`, form, {
      headers: {
