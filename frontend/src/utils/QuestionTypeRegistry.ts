...
const fullAnswerEvaluate = async (
  ...
  try {
    const ev: FullAnswerModelEvaluation = await OllamaService.gradeFullAnswer({
...
export const QUESTION_TYPE_HANDLERS: Record<TestQuestion['type'], QuestionTypeHandler<any>> = {
  single: {
    type: 'single',
