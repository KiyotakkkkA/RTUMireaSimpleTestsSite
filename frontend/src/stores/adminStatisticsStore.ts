...
export class AdminStatisticsStore {
    ...
    get statisticsAppliedFilters(): AdminStatisticsFilters {
        return {
            ...
            min_percentage:
                this.statisticsFilters.min_percentage === '' ? undefined : this.statisticsFilters.min_percentage,
        };
    }

    updateStatisticsFilters(next: Partial<AdminStatisticsFilters>): void {
        const updated = { ...this.statisticsFilters, ...next };
        if (isShallowEqual(updated as Record<string, any>, this.statisticsFilters as Record<string, any>)) return;
  ...
  async loadStatistics(): Promise<void> {
        try {
            ...
            const response = await AdminService.getStatistics(this.statisticsAppliedFilters);
            runInAction(() => {
                this.statisticsData = response;
            });
        } catch (e: any) {
            runInAction(() => {
                this.statisticsError = getErrorMessage(e, 'Не удалось загрузить статистику');
            });
        } finally {
            runInAction(() => {
